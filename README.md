...
#### 🛍️ Split Transaction Compatibility (e.g. `rr4444/actual-amazon-noter` and `rr4444/actual-ecommerce-noter` splits) [FORK FEATURE]

Fully supports and preserves split transactions during categorization. While the original `actual-ai` codebase would silently ignore or corrupt subtransaction structures when updating notes/categories, this fork correctly updates individual subtransaction entities (such as those generated by the [rr4444/actual-amazon-noter](https://github.com/rr4444/actual-amazon-noter) and [rr4444/actual-ecommerce-noter](https://github.com/rr4444/actual-ecommerce-noter) companion uploaders, which support auto-splitting), ensuring your itemized category splits remain perfectly intact and properly classified. It automatically parses and simplifies uploader note tags (such as `#Amazon-Product-Name`, `#PayPal-Item-Title`, and split index suffixes) into clean merchant descriptions for the LLM prompt.
...
- `amazonNoterWorkflow` - Ignore all Amazon/AMZN transactions (for use with actual-amazon-noter)
