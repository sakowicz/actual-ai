...
#### 🛍️ Split Transaction Compatibility (e.g. `rr4444/actual-amazon-noter` splits) [FORK FEATURE]

Fully supports and preserves split transactions during categorization. While the original `actual-ai` codebase would silently ignore or corrupt subtransaction structures when updating notes/categories, this fork correctly updates individual subtransaction entities (such as those generated by the [rr4444/actual-amazon-noter](https://github.com/rr4444/actual-amazon-noter) fork, which supports auto-splitting), ensuring your itemized category splits remain perfectly intact and properly classified.
