...
#### 🛍️ Split Transaction Compatibility (e.g. `actual-amazon-noter` splits) [FORK FEATURE]

Fully supports and preserves split transactions during categorization. While the original `actual-ai` codebase would silently ignore or corrupt subtransaction structures when updating notes/categories, this fork correctly updates individual subtransaction entities (such as those generated by tools like `actual-amazon-noter`), ensuring your itemized category splits remain perfectly intact and properly classified.
